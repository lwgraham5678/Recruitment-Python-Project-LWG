...
def ExpDistribution(n : int, max : int, beta : float):
    scalar = max
    Dis = np.random.exponential(beta, n)
    NDis = [scalar*round(x) for x in Dis]
    return NDis
# Test benches below
...
def ExpDistribution_TB(n : int, max : int, beta : float):

    L = ExpDistribution(n, max, beta)
...
def pipe():
    ...
    snd.playsound(r'C:\Users\lwgra\NetworkPython\env\LWGscripts\PipeFalling.mp3')